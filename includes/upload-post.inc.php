...
if (
	isset($_SESSION['post_Title']) &&
	isset($_SESSION['post_Description']) &&
	...
	if (
		isset($_SESSION['post_PathToImage']) &&
	...
	if (!mysqli_stmt_prepare($stmt, $sql)) {
		...
		if (!empty(mysqli_stmt_error($stmt))) {
			...
			header(
				'Location: ../admin.php?error=sqlError&errorMsg=' .
...
' .
						'https://dewandelmannen.nl/post.php?postId=' .
						$postId,
				];

				$res = $fb->post($id . 'feed/', $arr, $access_token);
				header(
					'Location: ../admin.php?notif=upload&notif2=fbUpload&postId=' .
						$postId .
						'&threadId=' .
				...
				header(
					'Location: ../admin.php?notif=upload&postId=' .
						$postId .
						'&threadId=' .
						mysqli_thread_id($conn)
				);
