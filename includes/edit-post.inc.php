...
if (
	...
	if (empty($postOldPathToPhoto)) {
		if (in_array(4, $_FILES['image']['error'])) {
			// DONT UPDATE PHOTO
			require 'dbh.inc.php';
			...
			if (!mysqli_stmt_prepare($stmt, $sql)) {
				...
				if (!empty(mysqli_stmt_error($stmt))) {
					...
					header(
						'Location: ../admin.php?notif=edit&postId=' .
							$postId .
							'&threadId=' .
		...
		} elseif (!in_array(4, $_FILES['image']['error'])) {
			// UPDATE PHOTO
			// delete old photo
	...
	} elseif (!empty($postOldPathToPhoto)) {
		if (in_array(4, $_FILES['image']['error'])) {
			// DONT UPDATE PHOTO
			require 'dbh.inc.php';
			...
			$sql =
				'UPDATE `posts` SET `title`=?, `description`=?,`contents`=?, `author`=?, `dateCreated`=? WHERE `id`=?;';
