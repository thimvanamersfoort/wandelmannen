...
const pluginsScripts = gulp.series(
	testPluginsScripts,
	function pluginsScripts() {
		return gulp
			.src(paths.pluginsScripts)
...
const styles = function () {
	return gulp
		...
		.pipe(
			$.header(banner, {
				pkg: pkg,
				description: "Default stylesheet for Trumbowyg editor",
			})
		)
		.pipe(gulp.dest("dist/ui/"))
...
const pluginsStyles = function () {
	return gulp
		...
		.pipe(
			$.header(banner, { pkg: pkg, description: "Trumbowyg plugin stylesheet" })
		)
		.pipe(
...
const pluginsSassDist = gulp.series(pluginsStyles, function pluginsSassDist() {
	return gulp
		...
		.pipe(
			$.header(banner, {
				pkg: pkg,
				description: "Default stylesheet for Trumbowyg editor plugin",
			})
		)
		.pipe(gulp.dest("dist/plugins"));
});
