(function ($) {
	...
	function renderGifs(response, $giphyModal, trumbowyg, mustEmpty) {
		...
		if (mustEmpty === true) {
			if (html.length === 0) {
				if (
					$("." + trumbowyg.o.prefix + "giphy-no-result", $giphyModal).length >
					0
				) {
					return;
				}
				...
				html =
					...
					trumbowyg.o.prefix +
					'giphy-no-result" src="' +
					trumbowyg.o.plugins.giphy.noResultGifUrl +
					'"/>';
			}
						...
						fn: function () {
							...
							var searchInput =
								...
								poweredByGiphy =
									'<div class="' +
									prefix +
									'powered-by-giphy"><span>Powered by</span>' +
									giphyLogo +
									"</div>",
								giphyModalHtml =
									searchInput +
							...
							var ajaxError = function () {
								if (
									!navigator.onLine &&
									!$("." + prefix + "giphy-offline", $giphyModal).length
								) {
									$giphyModal.empty();
									$giphyModal.append(
										'<p class="' + prefix + 'giphy-offline">You are offline</p>'
									);
								}
							};
							...
							var throttledInputRequest = trumbowygThrottle(
								searchGifsOnInput,
								trumbowyg.o.plugins.giphy.throttleDelay
							);
