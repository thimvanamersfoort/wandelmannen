	...
	function highlightIt(text, language, lineHighlight) {
		return [
			'<pre class="language-' +
				language +
				'" ' +
				(lineHighlight ? 'data-line="' + lineHighlight + '"' : "") +
				">",
			'<code class="language-' +
				language +
				'">' +
			...
			fn: function () {
				var $modal = trumbowyg.openModal(
						...
						[
							...
							'<div class="' + trumbowyg.o.prefix + 'highlight-form-group">',
							'   <input title="' +
								trumbowyg.lang.prismHighlightPluginAlert +
								'" placeholder="' +
								trumbowyg.lang.highlightLine +
								'" class="' +
								trumbowyg.o.prefix +
								'highlight-form-control trumbowyg-line-highlight"/>',
							"</div>",
						].join("\n")
					),
				...
				$modal.on("tbwconfirm", function () {
					...
					trumbowyg.execCmd(
						"insertHTML",
						highlightIt($code.val(), $language.val(), $lineHighlight.val())
					);
	...
	$.extend(true, $.trumbowyg, {
		...
		langs: {
			...
			en: {
				...
				prismHighlightPluginAlert:
					"You must have Prism Line Highlight plugin installed",
			},
			es: {
				highlight: "Resaltado de sintaxis de código",
				highlightLine: "Resaltar lineas, ej: 1,3-5",
				prismHighlightPluginAlert:
					"Debes de tener el plugin Prism Line Highlight instalado",
			},
			hu: {
