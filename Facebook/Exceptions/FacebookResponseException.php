	...
	public static function create(FacebookResponse $response)
	{
		...
		if (isset($data['error']['error_subcode'])) {
			switch ($data['error']['error_subcode']) {
				...
				case 467:
					return new static(
						$response,
						new FacebookAuthenticationException($message, $code)
					);
				...
				case 1363041:
					return new static(
						$response,
						new FacebookResumableUploadException($message, $code)
					);
				case 1363037:
					$previousException = new FacebookResumableUploadException(
						$message,
						$code
					);

					$startOffset = isset($data['error']['error_data']['start_offset'])
						? (int) $data['error']['error_data']['start_offset']
						: null;
					$previousException->setStartOffset($startOffset);

					$endOffset = isset($data['error']['error_data']['end_offset'])
						? (int) $data['error']['error_data']['end_offset']
						: null;
		...
		switch ($code) {
			...
			case 2:
				return new static(
					$response,
					new FacebookServerException($message, $code)
				);
			...
			case 613:
				return new static(
					$response,
					new FacebookThrottleException($message, $code)
				);
			...
			case 506:
				return new static(
					$response,
					new FacebookClientException($message, $code)
				);
		}
		...
		if ($code == 10 || ($code >= 200 && $code <= 299)) {
			return new static(
				$response,
				new FacebookAuthorizationException($message, $code)
			);
		}
