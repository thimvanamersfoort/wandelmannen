...
class GraphNodeFactory
{
	...
	public function makeGraphAchievement()
	{
		return $this->makeGraphNode(
			static::BASE_GRAPH_OBJECT_PREFIX . 'GraphAchievement'
		);
	}
	...
	{
		return $this->makeGraphNode(
			static::BASE_GRAPH_OBJECT_PREFIX . 'GraphAlbum'
		);
	}
	...
	{
		return $this->makeGraphNode(
			static::BASE_GRAPH_OBJECT_PREFIX . 'GraphSessionInfo'
		);
	}
	...
	{
		return $this->makeGraphNode(
			static::BASE_GRAPH_OBJECT_PREFIX . 'GraphEvent'
		);
	}
	...
	{
		return $this->makeGraphNode(
			static::BASE_GRAPH_OBJECT_PREFIX . 'GraphGroup'
		);
	}
	...
	public function validateResponseAsArray()
	{
		if (!is_array($this->decodedBody)) {
			throw new FacebookSDKException(
				'Unable to get response from Graph as array.',
				620
			);
		}
	}
	...
	public function safelyMakeGraphNode(array $data, $subclassName = null)
	{
		...
		foreach ($data as $k => $v) {
			...
			if (is_array($v)) {
				...
				$items[$k] = $this->castAsGraphNodeOrGraphEdge(
					$v,
					$objectSubClass,
					$k,
					$parentNodeId
	...
	public function castAsGraphNodeOrGraphEdge(
		...
		if (isset($data['data'])) {
			...
			if (static::isCastableAsGraphEdge($data['data'])) {
				return $this->safelyMakeGraphEdge(
					$data,
					$subclassName,
					$parentKey,
					$parentNodeId
	...
	public function safelyMakeGraphEdge(
		...
		if (!isset($data['data'])) {
			throw new FacebookSDKException(
				'Cannot cast data to GraphEdge. Expected a "data" key.',
				620
			);
		}
	...
	public static function validateSubclass($subclassName)
	{
		...
		throw new FacebookSDKException(
			'The given subclass "' .
				$subclassName .
				'" is not valid. Cannot cast to an object that is not a GraphNode subclass.',
			620
		);
	}
}
