...
class Facebook
{
	...
	public function __construct(array $config = [])
	{
		...
		if (!$config['app_id']) {
			throw new FacebookSDKException(
				'Required "app_id" key not supplied in config and could not find fallback environment variable "' .
		...
		if (!$config['app_secret']) {
			throw new FacebookSDKException(
				'Required "app_secret" key not supplied in config and could not find fallback environment variable "' .
		...
		$this->setUrlDetectionHandler(
			$config['url_detection_handler'] ?: new FacebookUrlDetectionHandler()
		);
	 ...
	 */
	private function setUrlDetectionHandler(
		UrlDetectionInterface $urlDetectionHandler
	) {
	...
	{
		return new FacebookJavaScriptHelper(
			$this->app,
			$this->client,
			$this->defaultGraphVersion
		);
	...
	{
		return new FacebookCanvasHelper(
			$this->app,
			$this->client,
			$this->defaultGraphVersion
		);
	...
	{
		return new FacebookPageTabHelper(
			$this->app,
			$this->client,
			$this->defaultGraphVersion
		);
	...
	public function uploadVideo(
		...
		$uploader = new FacebookResumableUploader(
			$this->app,
			$this->client,
			$accessToken,
			$graphVersion
